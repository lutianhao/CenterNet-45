...
def main():
  saver = create_saver(cfg.local_rank, save_dir=cfg.ckpt_dir)   #保存路径
  logger = create_logger(cfg.local_rank, save_dir=cfg.log_dir)  #日记路径
  ...
  num_gpus = torch.cuda.device_count()      #gpu数目
  if cfg.dist:          #这个应该是多gpus的时候运行的
  ...
  Dataset = COCO if cfg.dataset == 'coco' else PascalVOC   #初始化Dataset的类型，这里直接给dataSet了coco或者PascalVOC类
  train_dataset = Dataset(cfg.data_dir, 'train', split_ratio=cfg.split_ratio, img_size=cfg.img_size) #用于训练的数据集
  train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset,
                                                                  num_replicas=num_gpus,
                                                                  rank=cfg.local_rank) #采样，分布到不同的gpus上
  train_loader = torch.utils.data.DataLoader(train_dataset,
                                             ...
                                             pin_memory=True,
                                             drop_last=True,
                                             sampler=train_sampler if cfg.dist else None)  #数据读取
  ...
  val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1,
                                           ...
                                           collate_fn=val_dataset.collate_fn)  #读取到校验的数据了
  ...
  def train(epoch):
    print('\n Epoch: %d' % epoch)
    model.train()
    tic = time.perf_counter()  #这个应该是时间清零开始计时
